...
int head_read(ObjectID *id_out) {
    ...
    if (strncmp(line, "ref: ", 5) == 0) {
        char path[512]; snprintf(path, sizeof(path), "%s/%s", PES_DIR, line + 5);
        f = fopen(path, "r");
        if (!f) return -1;
...
int head_update(const ObjectID *new_commit) {
    ...
    line[strcspn(line, "\r\n")] = '\0';
    char path[512];
    if (strncmp(line, "ref: ", 5) == 0) snprintf(path, sizeof(path), "%s/%s", PES_DIR, line + 5);
    else strcpy(path, HEAD_FILE);
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    Commit c; memset(&c, 0, sizeof(Commit));
    if (tree_from_index(&c.tree) != 0) return -1;
    if (head_read(&c.parent) == 0) c.has_parent = 1;
    strncpy(c.author, pes_author(), sizeof(c.author));
